...
func (pow *ProofOfWork) ComputeData(nonce int) []byte {
	...
	//data == PrevHash+Data+nonce+difficulty
	data := bytes.Join([][]byte{[]byte(pow.Block.PrevHash),
		[]byte(pow.Block.Data),
		make([]byte, 8),
		make([]byte, 8)}, []byte{})

	binary.BigEndian.PutUint64(data[len(data)-16:],
		uint64(nonce))
...
func (pow *ProofOfWork) MineBlock() (int, []byte) {
	...
	for {
		...
		//comparing bytes of computedHash and targetValue
